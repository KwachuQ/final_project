...
- **Create**: Upload an inventory CSV (required) and an optional `name` field.
  The CSV is parsed and validated first; only after successful scoring is the
  file uploaded to S3 and results persisted to DB. The assessment is owned by
...
- **Database**: Test-scoped SQLite in-memory via dependency override.
- **S3 mocking**: `moto` mocks S3 in unit tests.
- **Fixtures** (`conftest.py`):
  - `db` — creates all tables, yields session, rolls back after each test.
...
5. The CSV is validated and scored before uploading to S3. If validation
   fails, nothing is stored — no orphaned files in S3.
